...
object ResetCooldownCommand {
    ...
    fun execute(sender: CommandSender, @Param(name = "player") player: Player, @Param(name = "kit") kit: Kit) {
        ...
        player.sendMessage("${KitsPlugin.instance.getChatPrefix()}${ChatColor.GREEN}Your ${kit.name} ${ChatColor.GREEN}kit cooldown has been reset!")
        sender.sendMessage("${KitsPlugin.instance.getChatPrefix()}${ChatColor.GREEN}You've reset ${ChatColor.RESET}${player.name}${ChatColor.GREEN}'s ${kit.name} ${ChatColor.GREEN}kit cooldown!")
    }
